import {CONVENIOS,MEDICACOES,MEDICOS,ARTICULACOES,LADOS,PROCESS_CONDITIONS,DOCUMENT_STATUSES,OPERATOR_STATUSES,RECEPTION_PENDING,FIELD_LABELS,processLabel} from './domain.js?v=14';
...
export function fillOptions(root=document,references=null,reset=false) {
  ...
  const workflowChoices={cidStatus:DOCUMENT_STATUSES,exameStatus:DOCUMENT_STATUSES,situacaoOperadora:OPERATOR_STATUSES,pendenciaRecepcao:RECEPTION_PENDING};
  for(const [name,values] of Object.entries(workflowChoices))for(const select of root.querySelectorAll(`select[name="${name}"]`)){
    if(reset)select.replaceChildren();
...
export function displayDateTime(value) {return value?new Intl.DateTimeFormat('pt-BR',{dateStyle:'short',timeStyle:'short'}).format(new Date(`${value}:00-03:00`)):'—';}
export function summary(container,fields,labels=FIELD_LABELS) {
  container.replaceChildren();
  for(const [key,label] of Object.entries(labels)) {
    const value=key==='numeroAplicacao' && fields[key] ? `${fields[key]}ª de 3` : key.startsWith('dataHora') && fields[key] ? displayDateTime(fields[key]) : key.startsWith('data') && fields[key] ? displayDate(fields[key]) : key==='condicaoProcesso' ? processLabel(fields) : fields[key];
